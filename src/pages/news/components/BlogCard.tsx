...
const BlogItem: React.FC<IBlogItemProps> = ({ blog }) => {
  ...
  return (
    <div className="border-b shadow-lg pb-12 rounded-[10px]">
      ...
      <div className="flex flex-col px-10 mt-2 gap-y-4">
        <p
          className="title-h4 !font-bold cursor-pointer hover:!text-secondary"
          onClick={() => handleNavigaeToNewDetail(blog.id)}
        >
          {blog.title}
        </p>
        <div className="line-clamp-2">{blog.subTitle}</div>
      </div>
    </div>
  );
};
